...
exports.getUser = [
    param('userId')
        .isInt().withMessage('المستخدم غير صالح'),
    requireValidation
]
...
exports.getUserOffers = [
    ...
    query('status')
        .optional()
        .isInt({min: 0, max: 2}).withMessage('حالة العرض غير صالحة'),
    query('lastOffered')
        .optional()
        .isBoolean().withMessage('حالة العرض غير صالحة'),
    query('isPaid')
        .optional()
        .isBoolean().withMessage('حالة العرض غير صالحة'),
    query('page')
...
exports.postUserReport = [
    ...this.getUser,
    check('details')
        .notEmpty().withMessage('رجاء توفير تفاصيل حول الإبلاغ'),
    requireValidation
]