...
exports.register = async (req, res, next) => {
    try {
        let user = await usersModel.getUserByPhone(req.body.phone);
        if (user)
            return next({ status: 400, msgs: ["هذا الحساب موجود من قبل"] });
...
exports.verify = async (req, res, next) => {
    try {
        ...
        if (!user)
            return next({ status: 401, msgs: ["هذا المستخدم غير مسجل"] });
        if (!(await usersModel.verifyCode(user, req.body.code)))
            return next({ status: 401, msgs: ["كود التفعيل غير صحيح"] });
